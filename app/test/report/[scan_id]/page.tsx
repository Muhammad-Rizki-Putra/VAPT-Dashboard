...
function RadarChart({ scores, cats }: { scores: Record<string, number>; cats: { key: string; label: string }[] }) {
  ...
  return (
    <svg viewBox="0 0 320 316" className="w-full max-w-xs mx-auto">
      <polygon points={poly(1)} fill="#f8fafc" className="dark:fill-zinc-800/40" />
      {[0.25, 0.5, 0.75, 1].map(t => (
        <polygon key={t} points={poly(t)} fill="none"
          stroke="#e2e8f0" strokeWidth={t === 1 ? 1.5 : 0.8}
          className="dark:stroke-zinc-600" />
      ))}
      {[25, 50, 75].map(pct => {
        const p = pt(-Math.PI / 2, pct / 100);
        return <text key={pct} x={p.x + 3} y={p.y} fontSize="8" fill="#94a3b8" className="select-none">{pct}%</text>;
      })}
      {angles.map((a, i) => {
        const e = pt(a, 1);
        return <line key={i} x1={cx} y1={cy} x2={e.x} y2={e.y} stroke="#cbd5e1" strokeWidth="1" className="dark:stroke-zinc-600" />;
      })}
      ...
      {cats.map((cat, i) => {
        ...
        return (
          <text key={i} x={p.x} y={p.y} textAnchor="middle" dominantBaseline="middle"
            fontSize="9.5" fill="#475569" className="dark:fill-zinc-400 select-none">
            {lines.map((line, j) => (
...
export default function ReportDetailPage() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64 text-slate-400 dark:text-zinc-500 gap-2 text-sm">
  ...
  if (error || !result) {
    return (
      <div className="max-w-xl mx-auto mt-20 space-y-4 text-center">
        ...
        <Link href="/test/history" className="text-sm text-blue-600 dark:text-red-400 hover:underline">
          {tr.backHistory}
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 transition-colors duration-300">
      ...
      <div className="flex justify-between items-center pb-4 border-b border-slate-200 dark:border-zinc-800">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-zinc-100 tracking-tight">{tr.title}</h1>
          <p className="text-sm text-slate-500 dark:text-zinc-400 mt-0.5">
            <Link href="/test/history" className="hover:text-blue-600 dark:hover:text-red-400 transition-colors">{tr.backHistory}</Link>
        ...
        <button
          ...
          className="bg-blue-600 dark:bg-red-500 text-white px-5 py-2.5 rounded-md text-sm font-medium hover:bg-blue-700 dark:hover:bg-red-600 transition-colors cursor-pointer flex items-center space-x-2 disabled:bg-slate-400 dark:disabled:bg-zinc-700 disabled:cursor-not-allowed shadow-sm"
        >
          {isPreparing
      ...
      <div
        ref={reportRef}
        className="bg-white dark:bg-zinc-900 p-10 border border-slate-200 dark:border-zinc-800 rounded-lg shadow-sm space-y-8 print:p-0 print:border-none print:shadow-none transition-colors duration-300"
      >

        {/* Title */}
        <div className="text-center pb-6 border-b-2 border-blue-600 dark:border-red-500">
          <h1 className="text-2xl font-bold tracking-wider text-slate-900 dark:text-zinc-100 uppercase">{tr.formalTitle}</h1>
          <p className="text-sm text-slate-600 dark:text-zinc-400 mt-1">{tr.formalSubtitle}</p>
          <p className="text-xs text-slate-500 dark:text-zinc-500 mt-0.5">{tr.mode}: {result.testing_mode}</p>
        </div>

        {/* Metadata */}
        <div className="overflow-hidden border border-slate-300 dark:border-zinc-700 rounded-sm break-inside-avoid">
        ...
        <div className="space-y-4 break-inside-avoid">
          <h2 className="text-lg font-bold text-blue-600 dark:text-red-500 uppercase tracking-wide">{tr.s1Title}</h2>
          <p className="text-sm text-slate-700 dark:text-zinc-300 leading-relaxed text-justify">{result.exploit_summary}</p>
          <div className="grid grid-cols-4 border border-slate-300 dark:border-zinc-700 rounded-sm overflow-hidden text-center">
        ...
        <div className="space-y-5 break-inside-avoid">
          <h2 className="text-lg font-bold text-blue-600 dark:text-red-500 uppercase tracking-wide">{tr.s2Title}</h2>

          <div className="grid grid-cols-2 gap-6 items-start">
            {/* Grade Badge */}
            <div className={`flex flex-col items-center p-6 border-2 rounded-xl ${gradeBase.bg} ${gradeBase.border} space-y-3`}>
              <p className="text-[10px] font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-widest">{tr.securityGrade}</p>
              ...
              <p className="text-xs text-slate-600 dark:text-zinc-400 text-center leading-relaxed">{gradeConf.desc}</p>

              <div className="w-full pt-3 border-t border-slate-200 dark:border-zinc-700 space-y-1.5">
                {radarCats.map(cat => {
                  ...
                  return (
                    <div key={cat.key}>
                      <div className="flex justify-between items-center mb-0.5">
                        <span className="text-[10px] text-slate-500 dark:text-zinc-400">{cat.label.replace('\n', ' ')}</span>
            ...
            <div className="flex flex-col items-center space-y-3">
              ...
              <div className="flex gap-5 text-[10px] text-slate-500 dark:text-zinc-400">
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-bold text-blue-600 dark:text-red-500 uppercase tracking-wide break-inside-avoid">{tr.s3Title}</h2>
          <div className="overflow-x-auto border border-slate-300 dark:border-zinc-700 rounded-sm">
            <table className="w-full text-sm text-left text-slate-700 dark:text-zinc-300">
              <thead className="bg-blue-600 dark:bg-red-800 text-white table-header-group">
                <tr>
                  <th className="px-4 py-2.5 font-semibold text-center border-r border-slate-300/20 w-12">{tr.colNo}</th>
                  <th className="px-4 py-2.5 font-semibold border-r border-slate-300/20">{tr.colName}</th>
                  <th className="px-4 py-2.5 font-semibold text-center border-r border-slate-300/20 w-24">{tr.colSeverity}</th>
                  <th className="px-4 py-2.5 font-semibold text-center border-r border-slate-300/20 w-20">{tr.colCvss}</th>
                  <th className="px-4 py-2.5 font-semibold border-r border-slate-300/20 w-20">{tr.colSource}</th>
                  <th className="px-4 py-2.5 font-semibold">{tr.colEndpoint}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-300 dark:divide-zinc-700">
                {result.vulnerabilities.map((v) => (
                  <tr key={v.id} className="hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors break-inside-avoid">
                    <td className="px-4 py-2.5 text-center font-medium border-r border-slate-300 dark:border-zinc-700">{v.id}</td>
                    <td className="px-4 py-2.5 border-r border-slate-300 dark:border-zinc-700">{v.name}</td>
                    <td className="px-4 py-2.5 text-center border-r border-slate-300 dark:border-zinc-700">
                    ...
                    <td className="px-4 py-2.5 text-center text-xs border-r border-slate-300 dark:border-zinc-700">{cvssToRange(v.cvss_score)}</td>
                    <td className="px-4 py-2.5 text-xs border-r border-slate-300 dark:border-zinc-700">{v.source}</td>
        ...
        {/* 4. Recommendations */}
        <div className="space-y-4 break-inside-avoid">
          <h2 className="text-lg font-bold text-blue-600 dark:text-red-500 uppercase tracking-wide">{tr.s4Title}</h2>
          <div className="space-y-4">
            {result.vulnerabilities.map((v, idx) => (
              <div key={v.id} className="text-sm break-inside-avoid">
                <p className="font-bold text-slate-800 dark:text-zinc-200 mb-1">{idx + 1}. {v.name}</p>
                <p className="text-slate-600 dark:text-zinc-400 flex items-start">
                  <span className="mr-2 text-slate-400 dark:text-zinc-500 flex-shrink-0">→</span>
        ...
        {/* 5. Conclusion */}
        <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-zinc-800 break-inside-avoid">
          <h2 className="text-lg font-bold text-blue-600 dark:text-red-500 uppercase tracking-wide">{tr.s5Title}</h2>
          <p className="text-sm text-slate-700 dark:text-zinc-300 leading-relaxed text-justify">{conclusion}</p>
        </div>

        {/* Footer */}
        <div className="mt-12 pt-4 border-t border-slate-200 dark:border-zinc-800 text-center break-inside-avoid">
...
function MetaRow({ label, value }: { label: string; value: string }) {
  return (
    <tr className="border-b border-slate-300 dark:border-zinc-700 last:border-b-0 break-inside-avoid">
      <td className="bg-blue-600 dark:bg-red-800 text-white font-medium px-4 py-2.5 w-1/4">{label}</td>
      <td className="bg-slate-50 dark:bg-zinc-800/50 px-4 py-2.5 text-slate-800 dark:text-zinc-200">{value}</td>
    </tr>
  );
}
...
function MetricBlock({ label, count, bgColor }: { label: string; count: number; bgColor: string }) {
  return (
    <div className="flex flex-col border-r border-slate-300 dark:border-zinc-700 last:border-r-0 break-inside-avoid">
      <div className="bg-blue-600 dark:bg-red-800 text-white text-xs font-bold py-2 tracking-wider">{label}</div>
