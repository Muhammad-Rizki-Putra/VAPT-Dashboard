...
export default function HistoryPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 transition-colors duration-300">
      {/* Header */}
      <div className="flex justify-between items-center pb-4 border-b border-slate-200 dark:border-zinc-800 transition-colors duration-300">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-zinc-100">{t.history.title}</h1>
          <p className="text-sm text-slate-500 dark:text-zinc-400">{t.history.subtitle}</p>
        </div>
        <div className="flex items-center space-x-3">
          <button
            onClick={() => { setLoading(true); fetchScans(); }}
            className="text-sm text-slate-500 dark:text-zinc-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors cursor-pointer"
          >
            {t.history.refresh}
          </button>
          <Link href="/test/new">
            <button className="bg-blue-600 dark:bg-red-500 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-blue-700 dark:hover:bg-red-600 transition-colors cursor-pointer shadow-sm">
              {t.history.newTest}
            </button>
          </Link>
        </div>
      ...
      {/* Table */}
      <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-lg shadow-sm overflow-hidden transition-colors duration-300">
        {loading ? (
          <div className="flex items-center justify-center py-16 text-slate-400 dark:text-zinc-500 text-sm gap-2">
          ...
          <div className="flex flex-col items-center justify-center py-16 text-slate-400 dark:text-zinc-500 text-sm space-y-2">
            <p>{t.history.empty}</p>
            <Link href="/test/new" className="text-blue-600 dark:text-red-400 hover:underline">
              {t.history.emptyLink}
            </Link>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left text-slate-600 dark:text-zinc-300">
              <thead className="bg-slate-50 dark:bg-zinc-800/50 text-slate-700 dark:text-zinc-200 transition-colors duration-300">
                <tr>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800 transition-colors duration-300">
                {scans.map((row) => (
                  <tr key={row.scan_id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors duration-200">
                    <td className="px-5 py-4 text-xs text-slate-500 dark:text-zinc-400 whitespace-nowrap">{formatDate(row.started_at)}</td>
                    ...
                    <td className="px-5 py-4">
                      {row.findings ? (
                        <div className="flex space-x-1.5 text-xs font-semibold">
                          <span className="px-1.5 py-0.5 rounded bg-blue-100 dark:bg-red-900/30 text-blue-700 dark:text-red-400" title="Total">{row.findings.total}</span>
                        ...
                        <span className="text-slate-400 dark:text-zinc-600 italic text-xs">—</span>
                      )}
                    </td>
                    <td className="px-5 py-4 text-right">
                      {row.status === 'completed' ? (
                        <Link href={`/test/report/${row.scan_id}`}>
                          <button className="text-blue-600 dark:text-red-400 font-medium hover:underline text-sm cursor-pointer transition-colors">
...
function StatusBadge({ status, progress, labels }: { status: string; progress: number; labels: StatusLabels }) {
  const map: Record<string, { style: string; label: string }> = {
    ...
    running:   { style: 'bg-blue-100 dark:bg-red-900/30 text-blue-700 dark:text-red-400 border-blue-200 dark:border-red-800/50',   label: `${labels.statusRunning} ${progress}%` },
    started:   { style: 'bg-blue-100 dark:bg-red-900/30 text-blue-700 dark:text-red-400 border-blue-200 dark:border-red-800/50',   label: labels.statusStarted },
  ...
  const { style, label } = map[status] ?? {
    style: 'bg-slate-100 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 border-slate-200 dark:border-zinc-700',
    label: status,
  };
